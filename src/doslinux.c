...
unsigned _dos_open(const char *fname, unsigned mode, int *handle)
{
 int file;
...
unsigned _dos_findnext(struct find_t *f)     //translate dos-style findfirst/next()
...
unsigned _dos_findfirst(char *name,unsigned start,struct find_t *f)
 {
   ...
   if (!p) f->path[p++]='.';
   f->path[p]=0;
   while ((name[p]=='/') || (name[p]=='*')) p++;
   strcpy(f->fname,name+p);
   p=strlen(f->fname);
   while ((!!p) && f->fname[p-1]=='*') p--;
   f->fname[p]=0;
...
unsigned _dos_getftime(int file, unsigned short *d, unsigned short *t)
{
     time_t utime;
     struct tm * tm;
...
void _dos_getdate(struct dosdate_t *t)
{
    time_t utime;
    struct tm * tm;
    time(&utime);
...
char *itoa( int value, char *string, int radix )
{
   ...
   char *p=string;
   *p=0;
   if (radix!=10) return string;        /* radix other than 10 not implemented */
