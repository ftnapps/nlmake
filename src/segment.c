...
process_segment (void)
{
  ...
  str[0] = 0;
  sprintf (str,
	   ";A %s Nodelist for %s, %s %0.1d, %d -- Day number %0.3d : 00000\r\n",
...
test_crc (char *filename, char *logline)
{
  ...
  if (listedcrc != 0)
	...
	{
	  sprintf (logline, "CRC Fail File: %0.5hu Actual: %0.5hu", listedcrc,
		   actualcrc);
//              printf("CRC Error!\n");
	  if (fp)
	    fclose (fp);
  ...
  sprintf (logline, "CRC Check File: %0.5hu Actual: %0.5hu", listedcrc,
	   actualcrc);
  if (fp)
    fclose (fp);
  return (0);
