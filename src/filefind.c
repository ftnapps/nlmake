...
char * extchr(char * string, char dot)
{
...
short FindMostCurr(char *FileName)
{
        long rc;
        char *dot;
        short Jdate=0;
...
//      printf("%s %u %u %u\n",fileinfo.name,fileinfo.wr_time, fileinfo.wr_date, fileinfo.size);
        ...
        if(dot != NULL)
        {
        if(Jdate <=  atoi(dot+1) && atoi(dot+1) <= 366 && filedate <= fileinfo.wr_date )
        {
        Jdate = atoi(dot+1);
        filedate = fileinfo.wr_date;
        }
