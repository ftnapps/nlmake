...
//      char newext[5];
  ...
  while(1)
  {
      if (oldeof == ftell (olist)) break; // hard error
                memset(oldstr,0,sizeof(oldstr));
                fgetpos(olist,&position);
      fgets (oldstr, MAXSTR, olist);
                if(strnicmp(oldstr,";A",2) != 0) break;
                if(oldstr[0] == 0) break;
                del++;
  }
  ...
  while(1)
  {
      if (neweof == ftell (nlist)) break; // hard error
                memset(newstr,0,sizeof(newstr));
                fgets (newstr, MAXSTR, nlist);
                if(strnicmp(newstr,";A",2) != 0) break;
                if(newstr[0] == 0) break;
                add++;
  }
  ...
  while(1)
  {
        ...
        if (neweof != ftell (nlist))
                ...
                if(newstr[0] == 0 && oldstr[0] == 0) break;
                else
                copy++;
                } else
         if(matchline < 0)
...
long find_match(void)
{
      ...
      if (neweof == ftell (nlist))
           ...
           //printf("Seek 3%d\n",resetcntr);
                ...
                //printf("newstr - %s\n",newstr);
                //printf("cntr - %d\n",linecnt);
                if(strcmp(oldstr,newstr) == 0) break;
                else
                linecnt++;
                }
...
short apply_diff(char *filename,short SFI)
{
   ...
   if(Findiff(diffname) == 1) return(1);
   ...
   fgets (diffstr, MAXSTR, diff);
        ...
        if(olist == NULL)
        {
        sprintf(logline,"Can not find segment <%s> to apply diff to",localname);
        logtext(logline,5,YES);
        fclose(diff);
        fclose(olist);
        fclose(nlist);
  ...
  //fseek (olist, 0L, SEEK_SET);
        ...
        while(1)
        {
      if (difeof == ftell (diff)) break;
                memset (diffstr, 0, sizeof(diffstr));
        fgets (diffstr, MAXSTR, diff);
                if(diffstr[0] == 0) break;
                switch(diffstr[0])
                {
                        case 'A':
                        add = atoi(diffstr+1);
...
short merge_list(char *filename)
{
         ...
         if(MAKETYPE == 6) return(1);
         //FindMostCurr(filename);

         if(MAKETYPE == 3) lmaketype = 2;
         else
                  lmaketype = MAKETYPE;
// printf("Nodelist       %s\n",filename);
 ...
 // printf("Temp Nodelist %s\n",filename);
      ...
      fgets (str, MAXSTR, Nodelst);
        ...
        // If we're a hub or node scan for us in segment
         if(lmaketype <= 1)
         {
          if(lmaketype == 1)
                ...
                fgets (str, MAXSTR, Nodelst);
                      if(str[0] == 0) break;
                                if(strnicmp(str,"ZONE",4) == 0) break;
                                if(strnicmp(str,"REGION",6) == 0) break;
                                if(strnicmp(str,"HOST",4) == 0) break;
                                if(strnicmp(str,cpstr,strlen(cpstr)) != 0)
                        fprintf(temp,"%s",str);
                                else
                                        break;
         ...
         // Now copy the new segment from our submitle file in to the new list
                        ...
                        {
                memset (str, 0,sizeof(str));
                   if(fgets (str, MAXSTR, outseg) == NULL) break;
       ...
       fprintf(temp,";\n");
        ...
        {
      memset (str, 0,sizeof(str));
        if(fgets (str, MAXSTR, Nodelst) == NULL) break;
                if(strnicmp(str,cpstr,strlen(cpstr)) == 0)
                break;
        }
