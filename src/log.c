...
short openlog(void)
{
  ...
  LogFile = fopen ("nlmake.log", "at+");
        ...
        {
         if(errno <= 39)
         sprintf(logline,"Critial Error - Can not open nlmake.log <%s>",errnostr[errno]);
         else
...
void logwrite(short message,short indicator)
{
  ...
  if(message == SYS_START) fprintf(LogFile,"\n");
  ...
  switch(LogMsgs[message].Type)
  {
                case 0: // Control File Errors
                        if(indicator >= 1)
                                {
                                fprintf (LogFile, "%c %0.2d-%s-%0.2d %0.2d:%0.2d:%0.2d - %-45.45s Line #%d\n",
                                denote[LogMsgs[message].loglevel],
                        date.day, Months[date.month].Single, date.year,
                        time.hour, time.minute, time.second,
                        LogMsgs[message].String,indicator);
                                printf ("%-45.45s Line #%d\n",LogMsgs[message].String,indicator);
                                } else
                                        {
                                        fprintf (LogFile, "%c %0.2d-%s-%0.2d %0.2d:%0.2d:%0.2d - %-45.45s\n",
                                  denote[LogMsgs[message].loglevel],
                                date.day, Months[date.month].Single, date.year,
                                time.hour, time.minute, time.second,
                ...
                case 2: // Control File Debug
                        fprintf (LogFile, "%c %0.2d-%s-%0.2d %0.2d:%0.2d:%0.2d - %-18.18s",
                   denote[LogMsgs[message].loglevel],
                date.day, Months[date.month].Single, date.year,
                time.hour, time.minute, time.second,
                LogMsgs[message].String);
...
void logtext(char *string, short indicator, short dateon)
{
  ...
  if(dateon == YES)
                        {
                        fprintf (LogFile, "%c %0.2d-%s-%0.2d %0.2d:%0.2d:%0.2d - %s\n",
                   denote[indicator],
                date.day, Months[date.month].Single, date.year,
                time.hour, time.minute, time.second,
                        string);
