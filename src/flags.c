...
void cre_flags_db (char *filename)
{
         char flagstr[512];
         FILE *flaglst;
  ...
  flaglst = fopen(filename,"rt");
        ...
        *replacelst = ',';
        ...
        memset (flagstr, 0,sizeof(flagstr));
           if(fgets (flagstr, 254, flaglst) == NULL) break;
                        //printf("flgstr %d: %s\n",flcnt++,flagstr);
                        flcnt++;
...
char * estrstr (char *haystack,char *needle)
{
 unsigned short i,j,len=strlen(needle);
 if (!len) return haystack;             /* if search string empty, return */
   ...
   if (haystack[i]==needle[0])          /* 1st chr must match - no wildcards */
    {
      ...
      for (j=0;j<len && haystack[i+j] && haystack[i+j]==needle[j];j++);
                        ...
                        if(fptr != NULL)
                                {
                                        ...
                                        if (strstr(quicklst,quickchk) != NULL)
                                          ...
                                          printf("Ignoring duplicate flag <%s> on line %d",fptr,flcnt);
                                        } else //
                                                {
                                              if(strlen(fptr) <= 48)
                                                        {
        ...
        if(strnicmp(fptr,"UFLAGS",6) == 0)
         ...
         if(strnicmp(fptr,"RFLAGS",6) == 0)
         {
           fptr = strtok(NULL,delims); // redundant flags
                if(fptr != NULL)
                {
                        ...
                        {
                   printf("Ignoring duplicate redundant flag <%s> on line %d",fptr,flcnt);
                        } else
                                  {
                                        // forced add
         ...
         if(strnicmp(fptr,"OFLAGS",6) == 0)
         {
                ...
                if(fptr != NULL)
                {
                        ...
                        {
                   printf("Ignoring duplicate replacement flag <%s> on line %d",fptr,flcnt);
                        // logtext
                        } else
                                  {
                                        // forced add
...
short checkflags(char *allflags)
{
        char localflags[255];
        char quickchk[255];
        char hue = 0;
                                                                ...
                                                                strcat(rplcdflags," -> ");
                                                                        ...
                                                                        set_redundant(quickchk,quickchk,umode);
                                                                                        ...
                                                                                        if(Ushift_bad(quickchk,umode) == 0)
                                                                                         {
                                                                                                if(umode == 0 || *fptr == 'U') strcat(badflags,",");
                                                                                                else strcat(badflags,",U");
                                                  ...
                                                  } else
                                                                  {
                                                                        ...
                                                                        quickchk[strlen(quickchk)-1] = 0;
                                                                        if(umode == 0 || *fptr == 'U') strcat(caseflags,",");
                                                                        else strcat(caseflags,",U");
...
//                printf("Redun Flags : %s\n",redunflags);
                  ...
                  if(*badflags == 0 && *caseflags == 0 &&
                    *dupeflags == 0 && *redunflags == 0 &&
                         *rplcdflags == 0) return(0);
...
short is_used(char *current, char *fptr, char *field)
{
        ...
        for(i=0;i<=100;i++)
        {
           //printf("3 Flag <%d> : %s\n",i,gflag[i].flag);
                ...
                retvalue = 0;
                gflag[i].ok = 0;
                if(*field == '*') gflag[i].dependnc = 0;
                else gflag[i].dependnc = 1;

                if(current[0] == 'G' || current[0] == '#')
                {
                strcpy(gflag[i].flag,fptr);
                } else
                                if(current[0] == 'T' && strlen(fptr) == 3)
                                {
                                strcpy(gflag[i].flag,fptr);
                                } else
                                                if(current[0] == 'I' && strlen(fptr) == 3)
                                                {
...
void set_redundant(char *quickchk, char *fptr, char umode)
{
        char *chptr;
                                         ...
                                         if(strstr(redunlst,quickchk) == NULL)
                                         {
                                           if(*quickchk == ',')
                                                ...
                                                {
                                                if(umode == 0 || *fptr == 'U') strcat(dupeflags,",");
                                                else strcat(dupeflags,",U");
...
void delete_redundant(char *field)
{
        short i=0;
        char *delims = { ",\r\n" };
        char *fptr;
                ...
                while(fptr != NULL)
                {
                        while(gflag[i].flag[0] != 0)
                         {
                                if(stricmp(gflag[i].flag,fptr) == 0) gflag[i].ok = 1;
...
void poundflag_test(void)
{
        ...
        if(pndptr == NULL || strlen(look_for_flag) == 3) return;
        if(look_for_flag[1] == '?')
        {
                look_for_flag[1] = 0;
                return;
        ...
        //printf("checking %s\n",quickchk);
        if(strstr(quicklst,quickchk) == NULL) break;
